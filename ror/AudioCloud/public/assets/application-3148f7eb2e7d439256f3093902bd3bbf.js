...
(function() {
  ...
  AudioPlayer = (function() {
    function AudioPlayer(params) {
      ...
      this.playFirstTrack = __bind(this.playFirstTrack, this);
      this.tunePlayPauseButtons = __bind(this.tunePlayPauseButtons, this);
      that = this;
      soundManager.url = params['url'] || '/sm2_swf/';
      soundManager.defaultOptions.autoPlay = true;
      soundManager.preferFlash = false;
      soundManager.ontimeout(function() {
        return console.error("SoundManager: I can't start.");
      });
      $(document).on('click', '[data-behavior~=player_switch_to_prev_track]', function(e) {
        return that.prevTrack();
      });
      $(document).on('click', '[data-behavior~=player_switch_to_next_track]', function(e) {
        return that.nextTrack();
      });
      $(document).on('click', '[data-behavior~=play_audio_track]', function(event) {
        event.preventDefault();
        if (!that.checkIfMusicCanBePlayed()) {
          console.log("SM2: I can't play audios");
          return false;
        }
        return that.playTrack(this);
      });
      $(document).on('click', '[data-behavior~=play_first_or_existing_track_on_page]', function(event) {
        event.preventDefault();
        if (!that.checkIfMusicCanBePlayed()) {
          console.log("I can't play audios");
          return false;
        }
        return that.togglePlayPauseOrPlayFirstTrack();
      });
      soundManager.onready(function() {
        var tracklist;
        return tracklist = $('[data-audio-track]');
      });
    }

    AudioPlayer.prototype.checkIfMusicCanBePlayed = function() {
      if (!soundManager.ok()) {
    ...
    AudioPlayer.prototype.tunePlayPauseButtons = function(element) {
      var i, yea;
      i = $(element).find('i');
    ...
    AudioPlayer.prototype.setAllPlayPauseButtonsToPlay = function() {
      return $('[data-track-container] i').removeClass('icon-pause').addClass('icon-play');
    };

    AudioPlayer.prototype.togglePlayPauseOrPlayFirstTrack = function() {
      var audioContainer, currentTrackId, elem, firstTrack, firstTrackId, id, sound, that, thereIsSomePlayingTrack, _ref;
      that = this;
      ...
      if (!thereIsSomePlayingTrack) {
        that.playFirstTrack();
        return console.log('playing first track');
      }
    };

    AudioPlayer.prototype.playFirstTrack = function() {
      var firstTrackElement;
      firstTrackElement = $('[data-audio-track]:eq(0)');
      firstTrackElement.click();
      return console.log('playing first thack (no previous track found)');
    };

    AudioPlayer.prototype.playTrack = function(element) {
      var currentTrack, data, err, id, sound, trackId, trackSource, trackTitle, _ref, _this;
      _this = this;
      data = $(element).data();
      trackId = data.id;
      ...
      this.tunePlayPauseButtons(element);
      try {
        $('.current_track_title').text(trackTitle);
      } catch (_error) {
        err = _error;
      }
      this.setNextPrevTrackElements(element);
      if (currentTrack) {
        _ref = soundManager.sounds;
        for (id in _ref) {
          sound = _ref[id];
        ...
        soundManager.createSound({
          ...
          onfinish: function() {
            var playFirstTrackIfNoNextTrack;
            _this.setAllPlayPauseButtonsToPlay();
            return _this.nextTrack(playFirstTrackIfNoNextTrack = false);
          }
        }).play();
      }
      return _this.currentTrackId = trackId;
    };

    AudioPlayer.prototype.nextTrack = function(playFirstTrackIfNoNextTrack) {
      if (playFirstTrackIfNoNextTrack == null) {
        playFirstTrackIfNoNextTrack = true;
      }
      if (this.nextTrackElement) {
        return this.nextTrackElement.find('[data-audio-track]').click();
      } else {
    ...
    AudioPlayer.prototype.prevTrack = function() {
      if (this.prevTrackElement) {
        return this.prevTrackElement.find('[data-audio-track]').click();
      } else {

      }
    };

    AudioPlayer.prototype.setNextPrevTrackElements = function(currentTrackElement) {
      var audioContainer, indexOfTrackInPlaylist, nextTrack, nextTrackIndex, prevTrack, prevTrackIndex, trackContainer;
      ...
      if (nextTrack.length > 0) {
        this.nextTrackElement = nextTrack;
      } else {
        this.nextTrackElement = null;
      }
      if (prevTrack.length > 0) {
        return this.prevTrackElement = prevTrack;
      } else {
        return this.prevTrackElement = null;
      }
    };

    AudioPlayer.prototype.getAudioContainer = function() {
      return $('[data-music-container]');
    };

    AudioPlayer.prototype.humanizeSeconds = function(seconds) {
      var minutes;
    ...
    AudioPlayer.prototype.updateTime = function(obj) {};
